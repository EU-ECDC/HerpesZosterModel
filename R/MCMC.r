...
for(i in 1 : nIter){
  ...
  for(j in 1 : length(prior[, 1])){
    param1[j] <- ifelse(param1[j] > prior[j, 2], 
                        prior[j, 2] - ((param1[j] - prior[j, 2]) %% (prior[j, 2] - prior[j, 1])),
                        ifelse(param1[j] < prior[j, 1], 
                               prior[j, 1] + ((prior[j, 1] - param1[j]) %% (prior[j, 2] - prior[j, 1])), 
                               param1[j]))
  }
  ...
  if(!is.na(lnLike1)){
    logAlpha <- (lnLike1 + dmvnorm(param0, param1, dispProp, log = TRUE)) - 
      (lnLike0 + dmvnorm(param1, param0, dispProp, log = TRUE)) 
...
cat("\nAlgorithm required", (proc.time()[3] - ptm) / 60, 
...
(p1 <- ggplot2::ggplot(data = reshape2::melt(sampledFoI, direction = "long",
                                            varying = list(2 : dim(sampledFoI)[2])),
                      mapping = aes(x = Var2, y = value, 
                                    group = Var1, col = Var1)) +
    ggplot2::geom_line() +
    ggplot2::theme(legend.position = "none"))

(p2 <- ggplot2::ggplot(data = reshape2::melt(sampledPrev, direction = "long",
                                            varying = list(2 : dim(sampledFoI)[2])),
                      mapping = aes(x = Var2, y = value, 
                                    group = Var1, col = Var1)) +
    ggplot2::geom_line() +
