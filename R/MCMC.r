...
FoI <- function(fitParams, otherParams, seroData){
  ...
  if(propFac == "constant"){
    if(length(fitParams) != 1){
      stop("length of fitParams does not fit choice of constant proportionality factor")
    }
  ...
  if(propFac == "loglin"){ # First option (age-dependent susceptibility)
    ...
    if(length(fitParams) != 2){
      stop("length of fitParams does not fit choice of proportionality factor")
    }
...
## MCMC function ##
###################
MCMC <- function(nIter = 2e3, # number of iterations per chain
                 init.scale.sd,	# set scaling factor for proposal distribution  
                 adapt.size.start,
                 adapt.size.cooling,
                 adapt.shape.start,
  ...
  ## Initialise chain ##
  ######################
  lnLike0 <- FoI(fitParams = param0, otherParams, seroData)$lnLike # Initialise log-likelihood
  ...
  summaryPrev <- sampledPrev %>% 
    ...
    mutate(width = width1) %>%
    mutate(point = "median")
  out <- list(mcmcOutput = mcmcOutput,
